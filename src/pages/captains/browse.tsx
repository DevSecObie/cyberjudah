...
type Episode = { title: string; url: string; date: string; year: string; thumb: string; books: string[]; teacher?: string };
...
export default function Browse() {
  const [all, setAll] = useState<Episode[] | null>(null);
  const [q, setQ] = useState("");
  const [years, setYears] = useState<string[]>([]);
  const [teachers, setTeachers] = useState<string[]>([]);
  const [sort, setSort] = useState<Sort>("new");
  const src = useBaseUrl("/search/captains.json");
  ...
  return (
    <Layout title="15 Minutes w/ The Captains" description="Browse the 15 Minutes w/ The Captains episode notes by year or name">
      <main className="cj-browse">
        <div className="cj-browse-inner">
          <header className="cj-browse-head">
            <h1>15 Minutes w/ The Captains</h1>
            <p>
              Short teachings from the captains, written up in full. Find one by name, or narrow to a year or teacher.
          ...
          <p className="cj-count">
            {all === null ? "Loading" : `${hits.length} ${hits.length === 1 ? "episode" : "episodes"}`}
          ...
          {all !== null && hits.length === 0 && (
            <p className="cj-empty">No episode by that name. <button type="button" onClick={clear}>Clear</button> and try again.</p>
          )}
        </div>
      </main>
    </Layout>
