...
export type Passage = { title: string; matchedTitle: string; excerpt: string; feed: string; date: string; video: string; start: number; note: string };
export const searchTeachings = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      const result = await env.DB.prepare(`SELECT title, highlight(teaching_passages,0,char(57344),char(57345)) AS matchedTitle, snippet(teaching_passages,1,char(57344),char(57345),' … ',48) AS excerpt, feed,date,video,start,note FROM teaching_passages WHERE teaching_passages MATCH ? AND (? = '' OR feed = ?) ORDER BY bm25(teaching_passages,4,1), rowid LIMIT 21 OFFSET ?`)
