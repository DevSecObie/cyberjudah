...
function Teachings() {
  ...
  return <Page>
    ...
    {!search.q && <div><p>Search words together, or use quotation marks for an exact phrase. Transcript captions may contain errors.</p><p style={{marginTop:'1rem'}}>Explore: {['forgiveness','Sabbath','Isaiah'].map(term=><Link key={term} to="/teachings" search={{q:term,feed:'',page:1}} style={{marginRight:'1rem'}}>{term}</Link>)}</p></div>}
    {result.unavailable ? <p role="status">Teaching search is currently unavailable. Please try again later. <Link to="/search" search={{q:search.q,only:undefined}}>Search published notes and scripture</Link></p> : search.q && <>
      <p className="cj-mono" role="status">{result.hits.length ? `Page ${search.page} · ${result.hits.length} matching passages` : 'No matching passages. Try fewer words or another collection.'}</p>
