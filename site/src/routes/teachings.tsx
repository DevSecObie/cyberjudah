...
export const Route = createFileRoute('/teachings')({
  validateSearch: (s: Record<string, unknown>) => ({ q: typeof s.q === 'string' ? s.q : '', feed: typeof s.feed === 'string' ? s.feed : '', page: Math.max(0, Math.floor(Number(s.page) || 0)) }),
  loaderDeps: ({ search }) => search,
  loader: ({ deps }) => searchTeachings({ data: deps }),
...
function timestamp(value: number) { const n = Math.max(0,Math.floor(Number(value)||0)); return `${Math.floor(n/60)}:${String(n%60).padStart(2,'0')}`; }
function Teachings() {
  ...
  return <Page>
    ...
    <form role="search" onSubmit={e => { e.preventDefault(); navigate({ to:'/teachings', search:{q:q.trim(),feed,page:0} }); }} style={{margin:'2rem 0'}}>
      ...
      <label style={{display:'block',marginTop:'1rem'}}>Collection <select value={feed} onChange={e=>setFeed(e.target.value)} style={{padding:'.6rem',marginLeft:'.75rem',color:'inherit',background:'var(--color-bg)'}}><option value="">All collections</option>{Object.entries(collections).map(([key,name])=><option key={key} value={key}>{name}</option>)}</select></label>
    </form>
    {!search.q && <div><p>Search words together, or use quotation marks for an exact phrase. Transcript captions may contain errors.</p><p style={{marginTop:'1rem'}}>Explore: {['forgiveness','Sabbath','Isaiah'].map(term=><Link key={term} to="/teachings" search={{q:term,feed:'',page:0}} style={{marginRight:'1rem'}}>{term}</Link>)}</p></div>}
    {result.unavailable ? <p role="status">Teaching search is currently unavailable. Please try again later. <Link to="/search" search={{q:search.q,only:undefined}}>Search published notes and scripture</Link></p> : search.q && <>
      <p className="cj-mono" role="status">{result.hits.length ? `Page ${search.page+1} · ${result.hits.length} matching passages` : 'No matching passages. Try fewer words or another collection.'}</p>
      {result.hits.map((hit,i)=><article key={`${hit.video}:${hit.start}:${i}`} style={{padding:'1.5rem 0',borderBottom:'1px solid var(--color-muted)'}}>
        ...
        <h2 style={{fontSize:'1.5rem',margin:'.5rem 0'}}><a href={`https://www.youtube.com/watch?v=${encodeURIComponent(hit.video)}&t=${Math.max(0,Math.floor(Number(hit.start)||0))}s`} target="_blank" rel="noreferrer">{hit.title}</a></h2>
        <p style={{lineHeight:1.7}}>{hit.excerpt}</p>
      ...
      <nav aria-label="Result pages" style={{display:'flex',gap:'2rem',marginTop:'2rem'}}>{search.page>0 && <Link to="/teachings" search={{...search,page:search.page-1}}>← Previous</Link>}{result.more && <Link to="/teachings" search={{...search,page:search.page+1}}>Next →</Link>}</nav>
    </>}
  </Page>;
}
