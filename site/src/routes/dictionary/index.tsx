...
function Dictionary() {
  ...
  return <Page>
    ...
    <form action="/dictionary" method="get" role="search" className="filters">
    ...
    <nav aria-label="Dictionary alphabet" className="era-jumps">
    ...
    <ul className="enc-list">{data.rows.map(row => <li key={row.slug}><Link to="/dictionary/$slug" params={{ slug: row.slug }}>{row.term}</Link></li>)}</ul>
